...
public class PrepareBattleMenuGUI {
	...
	public void buildGUI(Display display) {	
		...
		// create a new container for Ready button
	...
	class PlayerTanksTableModel implements ITableModel {
		...
		String[] header = {"menu.prepareBattle.tableHeader.playerName", 
						   "menu.prepareBattle.tableHeader.tankName", 
						   "menu.prepareBattle.tableHeader.tankModel",
						   "menu.prepareBattle.tableHeader.tankTeam"};
		String[][] matrix;
	                ...
	                public Object call() throws Exception {
	                	for (Player player : players) {	         
		        			...
		        			for (PlayerTank tank : player.getTanks()) {
		        				...
		        				// only local player can change your informations.
	                ...
	                public Object call() throws Exception {
	                	if (playerId == localPlayer.getId() && newSlotLocation != -1) {
		                	...
		                	if (mapOfPlayersIdAndMatrixIndex.containsKey(playerId)) {
		                		// change the team inside the tank instance of the player.
			...
			public void selectionChanged(SelectionChangedEvent arg0) {
				for (int i = 0; i < cbModelsArray.length; i++) {
					if (cbModelsArray[i] == owner) {
						...
						// check if tank's team really has been changed
						PlayerTank tank = localPlayer.getTankByName(matrix[i][TANKNAME_INDEX]);
						if (tank.getModel() != tm) {
							// set the new model inside localplayer tank instance.
							tank.setModel(tm);
					...
					for (int i = 0; i < cbTeamsArray.length; i++) {
						if (cbTeamsArray[i] == owner) {
							...
							if (tank.getTeam().getTeamEnum() != pt) {
								// change team in localPlayer tank
		...
		gameClient.addServerConnectionListener(
			new JGNConnectionListener() {
				...
				public void disconnected(JGNConnection con) {
					...
					mw.getOKButton().addButtonPressedListener(
					...
					mw.addWindowClosedListener(
						new IWindowClosedListener() {
	...
	private void addClientConnectionListener() {
		gameClient.addClientConnectionListener(
			new JGNConnectionListener() {
				public void connected(JGNConnection con) {}
					...
					new Thread(
						new Runnable() {
							public void run() {				
								...
								Player[] playerToAdd = new Player[otherOnlinePlayers.length + 1]; // others + localplayer
								
								// add the existent players
